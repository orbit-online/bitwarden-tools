...
longs=(--field --item --purpose); argcounts=(1 1 1); node_0(){ value __field 0
}; node_1(){ value __item 1; }; node_2(){ value __purpose 2; }; node_3(){
...
printf "%s\n" "${DOC:68:52}" >&2; exit 1; }'; unset var___field var___item \
var___purpose var_ANYARGS var___; parse 11 "$@"
local prefix=${DOCOPT_PREFIX:-''}; unset "${prefix}__field" "${prefix}__item" \
"${prefix}__purpose" "${prefix}ANYARGS" "${prefix}__"
...
"${prefix}__item" "${prefix}__purpose" "${prefix}ANYARGS" "${prefix}__"; done; }
