...
ret=$?; printf -- "exit %d\n" "$ret"; exit "$ret"; }; set -e
trimmed_doc=${DOC:0:452}; usage=${DOC:68:45}; digest=56bdb; shorts=(-p '')
longs=(--purpose --cache-for); argcounts=(1 1); node_0(){ value __purpose 0; }
node_1(){ value __cache_for 1; }; node_2(){ value ITEM a; }; node_3(){
value FIELD a; }; node_4(){ optional 0 1; }; node_5(){ optional 4; }; node_6(){
required 5 2 3; }; node_7(){ required 6; }; cat <<<' docopt_exit() {
[[ -n $1 ]] && printf "%s\n" "$1" >&2; printf "%s\n" "${DOC:68:45}" >&2; exit 1
}'; unset var___purpose var___cache_for var_ITEM var_FIELD; parse 7 "$@"
local prefix=${DOCOPT_PREFIX:-''}; unset "${prefix}__purpose" \
"${prefix}__cache_for" "${prefix}ITEM" "${prefix}FIELD"
...
"${prefix}__cache_for" "${prefix}ITEM" "${prefix}FIELD"; done; }
# docopt parser above, complete command for generating this parser is `docopt.sh --library='"$pkgroot/.upkg/andsens/docopt.sh/docopt-lib.sh"' bitwarden-value.sh`
  ...
  "$pkgroot/bitwarden-fields.sh" --cache-for "$__cache_for" --purpose "$__purpose" --json "$ITEM" "$FIELD" | jq -re --arg field "$FIELD" '.[$field]'
}
