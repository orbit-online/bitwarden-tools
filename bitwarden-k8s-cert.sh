...
Usage:
  bitwarden-k8s-cert [options] ITEMNAME

Options:
  --cache-for=SECONDS   Cache item with socket-credential-cache [default: 0]
"
# docopt parser below, refresh this parser with `docopt.sh bitwarden-k8s-cert.sh`
# shellcheck disable=2016,1090,1091,2034
docopt() { source "$pkgroot/.upkg/andsens/docopt.sh/docopt-lib.sh" '1.0.0' || {
ret=$?; printf -- "exit %d\n" "$ret"; exit "$ret"; }; set -e
trimmed_doc=${DOC:0:178}; usage=${DOC:45:46}; digest=1b1ff; shorts=('')
...
# docopt parser above, complete command for generating this parser is `docopt.sh --library='"$pkgroot/.upkg/andsens/docopt.sh/docopt-lib.sh"' bitwarden-k8s-cert.sh`
  eval "$(docopt "$@")"
  # shellcheck disable=2154
  eval "$("$pkgroot/bitwarden-fields.sh" -e --cache-for="$__cache_for" "$ITEMNAME" attachment:tls.crt attachment:tls.key)"
  # shellcheck disable=2154
  printf '{
