...
return 0; }; value() { local i; for i in "${!left[@]}"; do local l=${left[$i]}
...
printf -- "exit %d\n" "$1"; exit "$1"; }; set -e; trimmed_doc=${DOC:0:67}
...
# docopt parser above, complete command for generating this parser is `docopt.sh k8s-oidc.sh`
  eval "$(docopt "$@")"
  local data
