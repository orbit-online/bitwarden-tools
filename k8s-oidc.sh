...
# docopt parser above, complete command for generating this parser is `docopt.sh --library='"$pkgroot/.upkg/andsens/docopt.sh/docopt-lib.sh"' k8s-oidc.sh`
  eval "$(docopt "$@")"
  local data
  data=$("$pkgroot/bitwarden-fields.sh" --cache-for=900 "$ITEMNAME" oidc-issuer-url oidc-client-id oidc-client-secret)
  eval "$data"
