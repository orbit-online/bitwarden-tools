...
# docopt parser above, complete command for generating this parser is `docopt.sh --library='"$pkgroot/.upkg/andsens/docopt.sh/docopt-lib.sh"' k8s-cert.sh`
  eval "$(docopt "$@")"
  local data
  data="$("$pkgroot/bitwarden-fields.sh" --cache-for=900 "$ITEMNAME" attachment:tls.crt attachment:tls.key || echo return 1)"
  eval "$data"
