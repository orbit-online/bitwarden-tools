...
ret=$?; printf -- "exit %d\n" "$ret"; exit "$ret"; }; set -e
trimmed_doc=${DOC:0:848}; usage=${DOC:68:58}; digest=4b570; shorts=(-p '' '')
longs=(--purpose --field --item); argcounts=(1 1 1); node_0(){ value __purpose 0
}; node_1(){ value __field 1; }; node_2(){ value __item 2; }; node_3(){
...
printf "%s\n" "${DOC:68:58}" >&2; exit 1; }'; unset var___purpose var___field \
var___item var_ANYARGS var___; parse 11 "$@"; local prefix=${DOCOPT_PREFIX:-''}
unset "${prefix}__purpose" "${prefix}__field" "${prefix}__item" \
...
"${prefix}__field" "${prefix}__item" "${prefix}ANYARGS" "${prefix}__"; done; }
