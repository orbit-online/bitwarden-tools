...
ret=$?; printf -- "exit %d\n" "$ret"; exit "$ret"; }; set -e
trimmed_doc=${DOC:0:836}; usage=${DOC:68:58}; digest=c4a1f; shorts=('' -p '')
longs=(--field --purpose --item); argcounts=(1 1 1); node_0(){ value __field 0
}; node_1(){ value __purpose 1; }; node_2(){ value __item 2; }; node_3(){
...
printf "%s\n" "${DOC:68:58}" >&2; exit 1; }'; unset var___field var___purpose \
var___item var_ANYARGS var___; parse 11 "$@"; local prefix=${DOCOPT_PREFIX:-''}
unset "${prefix}__field" "${prefix}__purpose" "${prefix}__item" \
...
"${prefix}__purpose" "${prefix}__item" "${prefix}ANYARGS" "${prefix}__"; done; }
